...
public class XJPopupMenu extends JPopupMenu implements ActionListener {
	...
	private void init(JFrame jframe) {
		...
		scroll.setMinimumSize(new Dimension(490, 40));

		scroll.setMaximumSize(new Dimension(490, 500));
	...
	public void show(Component invoker, int x, int y) {
		...
		scroll.setPreferredSize(new Dimension(490, scroll
				.getPreferredSize().height));
		this.pack();
		this.setInvoker(invoker);
		if (sizescroll != 0) {
			//Set popup size only if scrollbar is visible
			this.setPopupSize(new Dimension(490, scroll.getMaximumSize().height - 20));
		}
